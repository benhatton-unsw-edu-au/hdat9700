...
if (lubridate::hour(Sys.time()) %in% 7:19 ) {
  low <- "#FFDB01"
  mid <- "#FEA923"
  high <- "#FD5E55"
  fontColor <- "grey10"  
...
ggplot(df, aes(x=x, label=lab, fill=x)) +
  geom_col(aes(y=y)) +
  geom_text(aes(y=base), color=fontColor) + 
  theme_void() +
  theme(legend.position = "none") +
  scale_fill_gradient2(low = low, mid = mid, high = high, midpoint=33/2) +
  theme(
